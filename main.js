...
Meteor.methods({
	...
	function getWaitingPlayer(name){
        var n=playerList.find({},fields:{username:1}}).fetch();
        var k=[playerList.find({},fields:{_id:1}}).fetch();
        for(var i=0;i<n.length;i++)
        {
        	if (n[i].isOnline && k[i]!=name ){
        		console.log("Match for game found:" + k[i]);
        		return true;
        		break;
        	}
        }
